...
def promethee(models_results):
    ...
    for criterion, direction in preferences.items():
        ...
        for model in models_results:
            if direction == "min":
                models_results[model][criterion] = (
                    max_value - models_results[model][criterion]
                ) / (max_value - min_value)
            elif direction == "max":
                models_results[model][criterion] = (
                    models_results[model][criterion] - min_value
                ) / (max_value - min_value)
    ...
    for model, metrics in models_results.items():
        scores[model] = sum(
            weights[criterion] * value for criterion, value in metrics.items()
        )
    print("Scores:", scores)
